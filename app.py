...
# ════════════════════════════════════════
    ...
    if horizon in ["3 ans"]: score -= 10
    elif horizon == "3 mois": score += 15
    score = max(0, min(100, score))

    if score < 30: couleur, profil, emoji = "#22c55e", "Conservateur", "Faible"
    elif score < 70: couleur, profil, emoji = "#f59e0b", "Equilibre", "Modere"
    else: couleur, profil, emoji = "#ef4444", "Agressif", "Eleve"
    ...
    with col2:
        if score < 30: st.info("Votre profil est conservateur. Une allocation de 1-3% en BTC est adaptee.")
        elif score < 70: st.warning("Votre profil est equilibre. Une allocation de 3-7% en BTC est recommandee.")
        else: st.error("Votre exposition est elevee. Envisagez une couverture partielle.")
    ...
    for alloc in allocations:
        ...
        rendement = poids_cash * 0.03 + poids_btc * 0.60
        volatilite = poids_btc * 0.70
        sharpe = (rendement - 0.03) / volatilite if volatilite > 0 else 0
        gain_eur = tresorerie_totale * rendement
        resultats_bench.append({"allocation": f"{alloc}% BTC", "rendement": rendement * 100, "sharpe": sharpe, "gain_eur": gain_eur})
    ...
    col1, col2, col3 = st.columns(3)
    col1.metric("Rendement estime", f"{bench_actuel['rendement']:.1f}%")
        ...
        with st.spinner("Recuperation des donnees historiques..."):
            try:
                ...
                if "prices" not in data:
                    ...
                    invest_btc = tresorerie_totale * alloc_back / 100
                    btc_achete = invest_btc / prix_depart
                    annees = jours_back / 365
                    valeur_fin = btc_achete * prix_fin + (tresorerie_totale * (1 - alloc_back/100)) * (1 + 0.03 * annees)
                    gain = valeur_fin - tresorerie_totale * (1 + 0.03 * annees)
                    col1, col2, col3 = st.columns(3)
                    ...
                    fig2.update_layout(title=f"Evolution du prix BTC sur {periode_back} (EUR)", height=350)
    ...
    elif nb_alertes <= 2: st.warning(f"{nb_alertes} alerte(s) active(s) — revoyez votre exposition.")
    else: st.error(f"{nb_alertes} alertes actives — action immediate recommandee.")
    ...
    fig_hedge.update_layout(title="Impact de la couverture sur le risque (EUR)", barmode="group", height=320)
    ...
    elif horizon == "6 mois": alloc_max = min(alloc_max, 5)
    if revenus_usd > 50: alloc_min += 1; alloc_max += 2
    ...
    if ecart_actuel > 0:
        col3.metric("Action recommandee", f"Acheter {ecart_actuel:,.0f} EUR de BTC")
        st.warning(f"Vous pourriez ajouter {ecart_actuel:,.0f} EUR de BTC pour atteindre l'allocation optimale.")
    elif ecart_actuel < 0:
        col3.metric("Action recommandee", f"Reduire de {abs(ecart_actuel):,.0f} EUR")
        st.warning(f"Votre exposition depasse l'optimal de {abs(ecart_actuel):,.0f} EUR.")
    else:
